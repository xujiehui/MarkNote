...
修复后的本地代码通过了自动化测试、生产构建、Electron 打包检查和浏览器主流程自测。提交 `1c667d4` 已推送，新的 Pages 构建、部署和线上校验三个阶段均已成功。
...
已验证线上 bundle 包含后端配置端点，没有嵌入 Supabase secret/service-role key，配置端点 CORS 正常，Google provider 已启用；新的 Pages workflow 还额外通过了 Supabase 同步后端探测。
...
本轮新增了 Pages 的 `Verify Supabase sync backend` 步骤。提交 `1c667d4` 对应的 Pages Build、Deploy、Verify deployed cloud sync 三个 job 全部 success。

提交 `1c667d4` 对应的 Desktop Build 也已完成：Lint and test、macOS Intel、macOS Apple Silicon、Windows x64 四个 job 全部 success；Publish GitHub release 因该提交不是 version tag 而 skipped。
...
- [x] 推送本轮修复并验证新的 Pages workflow
- [x] 确认本轮 Desktop Build 的 macOS/Windows 三个平台 job 全部 success
